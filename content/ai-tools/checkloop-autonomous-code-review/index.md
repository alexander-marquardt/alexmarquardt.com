...
There are 38 built-in checks (including two bookend checks that ensure the test suite is green before and after the review). Thirty-seven of them are organized into four execution plans of increasing depth; the last is an on-demand check covered further down:
...
**Super-exhaustive** (37 checks) — everything in exhaustive plus infrastructure and hygiene audits, a CONTRIBUTING-conformance audit, a recurring-issue history audit, and a final meta-review — passes too slow or too project-specific to run on every push:
...
- **`xhigh`** for the correctness-critical and source-of-truth checks — `security`, `concurrency`, and the boundary and deep-advisory cluster (`architecture-boundaries`, `derived-values`, `coherence`, `meta-review`, `recurring-issues`) — where the extra deliberation is exactly what catches the subtle cases.
